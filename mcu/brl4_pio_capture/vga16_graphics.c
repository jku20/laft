...
// pixels will be automatically updated on the screen.
 inline void drawPixel(short x, short y, char color) {
    ...
    // Which pixel is it?
    //int pixel = ((640 * y) + x) ;
    // make sure we are in the 640x480 range
    //if (pixel >= 307200) return ;

    // identify the memory location of the pixel
    //unsigned char* pixel_ptr = &vga_data_array[pixel>>1] ;
    //char* pixel_ptr = address_pointer + (pixel>>1);
    char* pixel_ptr = address_pointer + (320 * y) + (x>>1);

    // Is this pixel stored in the first 4 bits
    // of the vga data array index, or the second
    // 4 bits? Check, then mask.
    if (x & 1) {
       *pixel_ptr = (*pixel_ptr & TOPMASK) | (color << 4) ;
    } else {
       *pixel_ptr = (*pixel_ptr & BOTTOMMASK) | (color) ;
    }
}
